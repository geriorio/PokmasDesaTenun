...
class custom extends Command
{
    ...
    public function handle()
    {
        ...
        if (! $err) {
            $this->info("----------------------------------------\nAll Component from {$className} successfully made!");
        }
    }
    ...
    public function generateFile(
        ...
        try {
            ...
            if (strpos($e->getMessage(), 'No such file or directory') !== false) {
                $this->error("ERROR: Directory /{$componentPath}/{$classDirectory} not found.");
                $answer = $this->ask("Do you want to make new directory {$classDirectory} in {$componentPath}? [y/n]");

                if ($answer === 'y') {
                    $this->info("Creating new directory /{$componentPath}/{$classDirectory}...");
                    File::makeDirectory("{$componentPath}/{$classDirectory}", 0755, true);

                    $this->info("Creating {$templateFileNameTitleCase} {$className}...\n");
                    File::put($path, $content);
                } elseif ($answer === 'n') {
                    $err = true;
                } else {
                    $err = true;
                    $this->error('ERROR: Answer not valid.');
                    $this->info("\n");
                }
            } else {
                $err = true;
